...
(* gdy ile = 1 typ wartosc reprezentuje przedzial pojedynczy [jeden; dwa],
a gdy ile = 2 to przedzial podwojny [-nieskonczonosc; jeden] u [dwa; nieskonczonosc] *)
...
let zmiana a b operacja = 
    (* oblicza wynikowy przedzial dla danej operacji i dwoch przedzialow (niekoniecznie pojedynczych) *)
    
    let czy_nan x =
        (* sprawdza czy x jest liczba *)
        classify_float x = FP_nan
    in
    
    let wrzuc a lista = 
        (* umieszcza przedzial na liscie, jesli przedzial jest podwojny to dzieli go na dwa pojedyncze *)
        if a.ile = 1 then
            a::lista
        else
    ...
    let rec oblicz lista_a lista_b cala_lista_b lista_c = 
        (* oblicza wynik operacji kazdego przedzialu z lista_a z kazdym przedzialem z lista_b
        (lista_a, lista_b i wynikowa lista_c sklada sie wylacznie z przedzialow pojedynczych) *)
        ...
        | (glowa_a::ogon_a, glowa_b::ogon_b) -> 
            ...
            if czy_nan przedzial.jeden
            || czy_nan przedzial.dwa
            || czy_nan glowa_a.jeden 
            || czy_nan glowa_a.dwa 
            || czy_nan glowa_b.jeden
            || czy_nan glowa_b.dwa then
                [{jeden = nan; dwa = nan; ile = 1}]
            else
    ...
    let porownaj a b =
        (* komparator do sortowania przedzialow pojedynczych,
        potrzebny do sortowania niemalejaco po lewych koncach przedzialow *)
    ...
    let rec lacz lista1 poczatek koniec lista2 = 
        (* laczenie przedzialow (przedzialy majace czesc wspolna sa zamieniane w jeden przedzial bedacy ich suma) *)
        if lista1 = [] then
    ...
    let rec maksymalna_roznica lista wartosc1 wartosc2 = 
        (* oblicza maksymalna roznice miedzy kolejnymi przedzialami pojedynczymi dla zlaczona_lista *)
        match lista with
    ...
    (* lista_a zawiera jeden przedzial pojedynczy [a.jeden; a.dwa] lub [nan; nan] 
    lub dwa przedzialy pojedyncze [-nieskonczonosc; a.jeden], [a.dwa; +nieskonczonosc] *)
    
    let lista_b = wrzuc b [] in 
    (* lista_b analogicznie do lista_a *)
    
    let lista_pom = oblicz lista_a lista_b lista_b [] in
    (* lista_pom zawiera liste przedzialow pojedynczych o dlugosci maksymalnie 8,
    przedzialy te moga sie pokrywac *)
    ...
    if czy_nan glowa.jeden
    || czy_nan glowa.dwa
    || (glowa.jeden = neg_infinity && glowa.dwa = infinity) then 
        (* przypadki [-nieskonczonsc; nieskonczonosc] lub [nan; nan] *)
        glowa
    else 
...
let sr_wartosc a =
    ...
    || typ1 = FP_nan 
    || typ2 = FP_nan 
    || (typ1 = FP_infinite && typ2 = FP_infinite) then
        nan
    else
        (a.jeden +. a.dwa) /. 2.0
;;
